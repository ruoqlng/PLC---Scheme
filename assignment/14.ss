...
(define *prim-proc-names* '(append eqv? list-tail apply map + - * add1 sub1 cons
				   ...
				   cdaar cdadr cddar cdddr list null? eq? equal? length 
				   list->vector list? pair? vector->list vector vector-set! vector? vector-ref
				   number? symbol? procedure? member newline set-car! set-cdr!
...
(define syntax-expand
    (lambda (exp)
        (cases expression exp
            ...
            [let-exp (var body)
                (app-exp
                    (lambda-exp (map 2nd (map 1st var)) (map syntax-expand body)) (map syntax-expand (map 2nd var)))]
...
; begin, cond, and, or, let*, and case
;--------------------------------------+
;                                      |
