  ...
  (cond
    ...
    [(list? datum)
     (cond
        ...
        ;[(eqv? (1st datum) ':) (lexical-exp (cadr datum) (caddr datum))]
        [(eqv? (1st datum) 'case)
...
(define apply-env-lex
    (lambda (env address depth succeed fail)
        (deref (apply-env-ref-lex env address depth succeed fail))))
  ...
  (lambda (env address depth succeed fail)
    (cases environment env
	   [empty-env-record ()
			     (fail)]
	   [extended-env-record (syms vals env)
				(cond [(symbol? depth) (fail)]
				      [(= 0 depth) 
                        ;(display env) (newline)
				       (succeed (list-ref vals address))]
				      [else 
				       (apply-env-ref-lex env 
						      address
						      (- depth 1)
						      succeed
						      fail)])])))
...
(define syntax-expand
    (lambda (exp)
        (cases expression exp
            ...
            ;         [(null? (cdr body)) (syntax-expand (car body))]
            ;         [else 
            ;         (syntax-expand (if-else-exp boole (car body) (syntax-expand (or-exp (cdr body)))))])]
            ;[or-exp (body)
            ;(syntax-expand
                ; (if (null? body)
                ;     (lit-exp #f)
                ;     (if (null? (cdr body)) 
                ;         (map syntax-expand (car body))
                ; (let ([next (syntax-expand (car body))])
                ;      (if-else-exp next next (or-exp (cdr body))))))] 
            ...
            [if-else-exp (bool true false)
            (if-else-exp (syntax-expand bool) 
                 (syntax-expand true)
                 (syntax-expand false) )]
            [if-exp (bool true)
...
(define lexical
    (lambda (datum check)
        (let ((r (search datum check)))
            (if r 
                (var-exp (cons ': r))
            (var-exp (list ': 'free datum))))))
            
(define (update-lambda ls var-ls)
  (append (map (lambda (x)
		 (list x
		       (list 0 (pos x ls 0)))) ls) 
	  var-ls))

(define (pos sym ls num)
    (cond [(eq? sym (car ls)) num]
        [else (pos sym (cdr ls) (add1 num))]))
...
(define (search sym ls)
    (cond 
        [(null? ls) #f]
        [(eq? sym (caar ls)) (cadar ls)]
        [else (search sym (cdr ls))]))
	  ...
	  (lambda (exp check)
	    (cases expression exp
		   [var-exp (id)
			    (lexical id check)]
		   [define-exp (var body)
		     (define-exp var (lex body check))]
		   [set!-exp (id var)
			     (set!-exp (lexical id check) (lex var check))]
		   [app-exp (rator rand)
			    (app-exp (lex rator check)
				     (map (lambda (x)
					    (lex x check)) rand))]
		   ;[begin-exp (body)
			    ;  (begin-exp (map (lambda (x)
					;	(lex x check)) body))]
		   [while-exp (test body)
			      (while-exp test (map (lambda (x)
						     (lex x check)) body))]
		   [lambda-exp (id body)
			       (lambda-exp id 
					    (map (lambda (x)
						   (lex x (update-lambda id
									 (update check)))) body))]
		   [lambdai-exp (id body)
		    (lambdai-exp
		     (pair->list id)
		     (map (lambda (x)
			    (lex x 
				  (update-lambda (pair->list id)
						(update check)))) body))]
		   [if-else-exp (bool true false)
			     (if-else-exp (lex bool check)
				       (lex true check)
				       (lex false check))]

		   [if-exp (bool true)
			     (if-exp (lex bool check)
				       (lex true check))]
		   [else exp]))])
      (lex exp (list)))))
  ...
  (lambda (exp env)
    (cases expression exp
        ...
        [while-exp (test body)
          ...
          [set!-exp (id var)
            (if (symbol? (cadr (cadr id)))
			   (set-box!
			    (apply-env-ref 
			     global-env
              (caddr (cadr id))
              (lambda (x) x)
              (lambda () (eopl:error 'set! "Could not set global variable: ~s" id)))
			    (eval-exp var env))
			   (set-box!
			    (apply-env-ref-lex
			     env 
			     ...
			     (cadr (cadr id))
			     (lambda (x) x)
			     (lambda () (eopl:error 'set! "Could not set variable: ~s" id)))
          (eval-exp var env)))]
        [define-exp (var body)
