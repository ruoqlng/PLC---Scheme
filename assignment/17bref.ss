     ...
     (cond
        ...
        [(eqv? (1st datum) 'lambda)
            (cond
                ...
                [else
                    (cond
                    [(list-of scheme-value? (2nd datum)) 
                        (lambda-exp 
                            (2nd datum)
...
(define extend-env
    (lambda (syms vals env)
        (extended-env-record syms (map check-box vals) env)))
...
(define syntax-expand
    (lambda (exp)
        (cases expression exp
            ...
            [lambda-exp (id body) (lambda-exp (find-ref id) (map syntax-expand body))]
        ...
        [var-exp (id) 
            ...
            (lambda ()
                ...
                (lambda () ; call if id not in global-env
                        (eopl:error 'apply-env-var "variable ~s is not bound"
						       id)))))]
        [app-exp (rator rands)
        (display "app exp : ")
        (display rator) (newline)
        (let ([proc-value (eval-exp rator env)])
            (cases proc-val proc-value
            [closure (vars bodies env2)
            ;(display "enter lambda:  ")
				    ...
				    (lambda (x y) 
                      ...
                      ;) 
                        (cases expression y
                            ...
                            (lambda (x) x)
                            (lambda () 
                            (eopl:error 'apply-env-ref "variable ~s is not bound"  (cadr y)   )))))
                            ]

                            [else 
                            ;(display "expression y : ")
                            ;(display y) (newline)
                            (eval-exp y env)]
                        )))
                             vars rands)
        ...
        [while-exp (test body)
          ...
          [set!-exp (id var)
            ...
            (set-ref! (apply-env-ref env id 
                      ...
                      (lambda () (eopl:error 'apply-env-set "variable not found" id))) (eval-exp  (syntax-expand var) env))))
