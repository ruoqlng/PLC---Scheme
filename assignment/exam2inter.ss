...
(define parse-for-to
  (lambda (datum)
    (for-to-exp
      ...
      (map parse-exp (cadddr (cddddr datum))))))

(define parse-for-downto
  (lambda (datum)
    (for-downto-exp
      (2nd datum)
      (parse-exp (cadddr datum)) 
  ...
  (cond
    ...
    [(list? datum)
     (cond
        ...
        ;--------------------exam 2
        [(eqv? (1st datum) 'for) 
            (if (eqv? (cadr (cdddr datum)) 'to) (parse-for-to datum)
                (parse-for-downto datum))]
        ;--------------------exam 2
        [(eqv? (car datum) 'set!) 
...
(define syntax-expand
    (lambda (exp)
        (cases expression exp
            ;---------------exam 2
            [for-to-exp (vars start end body)
                (syntax-expand (let ((end-var (string->symbol (format "_ending~s" vars))))
                (named-let-exp 'loop
                    (list vars end-var)
                    (list start end)
                    (list 
                        (if-else-exp
                            (app-exp (var-exp '<=)
                            (list (var-exp vars) (var-exp end-var)))
                                (begin-exp 
                                    (append body (list (app-exp (var-exp 'loop) 
                                        (list (app-exp (var-exp '+) (list (lit-exp 1)
                                                      (var-exp vars)))
                                                (var-exp end-var))))))
                                                    (lit-exp #f))))))]
            [for-downto-exp (vars end start body)
                (syntax-expand (let ((end-var (string->symbol (format "_ending~s" vars))))
                (named-let-exp 'loop
                    (list vars end-var)
                    (list end start)
            ...
            [or-exp (body)
                (cond
                    [(null? body) (lit-exp #f)]
                    [(null? (cdr body)) (syntax-expand (car body))]
                    [else (syntax-expand (if-else-exp (car body) (car body) (syntax-expand (or-exp (cdr body)))))])]
...
(define eval-exp
  (lambda (exp env)
    (cases expression exp
        ...
        [set!-exp (id var) (set-box! (apply-env-ref env id (lambda (x) x)
                (lambda () ; call if id not in global-env
                        (eopl:error 'apply-env-ref "variable ~s is not bound"
						       id))) (eval-exp var env) )]
        [var-exp (id) 
            (apply-env 
            env 
